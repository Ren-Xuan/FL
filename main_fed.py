...
import torch
from utils.sampling import agnews_iid, mnist_iid, mnist_noniid, cifar_iid
...
if __name__ == '__main__':
    ...
    if args.dataset == 'mnist':
        ...
        if args.iid:
            dict_users = agnews_iid(dataset_length, args.num_users)
        else:
            exit('Error: only consider IID setting in CIFAR10')
    ...
    acc_tests = []
    indexWeights = [1 for _ in range(args.num_users)]
    print("each client has data len:"+str(len(dict_users[0])))
    ...
    for iter in range(args.epochs):
        ...
        if args.fedslice:
            w_glob =FedSlice(w_locals,indexWeight= indexWeights)
